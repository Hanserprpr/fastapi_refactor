...
async def register_user(db: Session, redis_conn: ConnRedis, name: str, email: str, sex: str, password: str, qq: str) -> Dict:
    connsql = Connsql(db)
    ...
    encrypted_password = passwd.encrypt(password)
...
async def login_user(db: Session, identifier: str, password: str) -> str:
    ...
    input_passwd = connsql.search_passwd(identifier)
    if not user or not passwd.decrypt(password, input_passwd):
